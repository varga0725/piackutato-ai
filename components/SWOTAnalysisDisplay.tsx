...
const SWOTPointForm: React.FC<SWOTPointFormProps> = ({ point, onSave, onCancel }) => {
    const [text, setText] = useState(point?.text || '');
    const [impact, setImpact] = useState<ImpactLevel>(point?.impact || 'MEDIUM');

    const handleSave = () => {
        if (text.trim()) {
            onSave({ text: text.trim(), impact });
        }
    };

    return (
        <div className="p-3 bg-muted/50 rounded-lg border border-border animate-fade-in-up" style={{ animationDuration: '0.3s' }}>
            <textarea
                value={text}
...
const Quadrant: React.FC<QuadrantProps> = ({ title, items, icon, colorClass, quadrantKey, competitors, onAdd, onEdit, onDelete, editingInfo, addingInfo, onSave, onCancel }) => (
  <div className={`rounded-lg p-4 h-full flex flex-col border border-border bg-card`}>
    ...
    <div className="divide-y divide-border flex-grow">
        {items.map((item, index) => {
            ...
            return editingInfo?.quadrant === quadrantKey && editingInfo.index === index ? (
                ...
                <div key={index} className="flex justify-between items-start py-3 text-sm">
                    <div className="relative group flex items-start gap-2 pr-2">
                        <p className="text-foreground leading-relaxed">{item.text}</p>
                         {competitor && (
                            <>
                                <span title={`Kapcsolódik: ${competitor.name}`}>
                    ...
                    <div className="flex items-center flex-shrink-0">
                        ...
                        <div className="flex items-center ml-2 opacity-0 hover:opacity-100 focus-within:opacity-100 transition-opacity">
    ...
    {addingInfo === quadrantKey ? (
        <div className="py-2 mt-2">
            <SWOTPointForm onSave={(point) => onSave(quadrantKey, point)} onCancel={onCancel} />
        </div>
    ) : (
